# MigrationRunner S3 Artifact 設計（初版）

## この文書の位置づけ

- `MigrationRunner` の SQL 配布方式を Lambda 同梱から S3 artifact へ切り替える設計
- 本文書は「開発/検証フロー」の設計を対象とする
- 本番運用ルール（承認フローや実行責務分離）は本設計のスコープ外

## 背景

- 現状は migration/seed SQL を Lambda artifact に同梱している
...
- 対象:
  - `OpsStack` に migration artifact 用 S3 バケットを追加
  - `MigrationRunnerFunction` が S3 から SQL artifact を取得して実行
...
- S3 方式へ一本化する
- SQL artifact は zip 1ファイルで扱う
- key は固定運用（上書き）を許容する
- artifact の bucket/key は Lambda 環境変数で固定する
...
2. zip を `MigrationArtifactBucket` の固定 key にアップロード
...
- `OpsStack` に `MigrationArtifactBucket` を追加
- 設定:
...
- payload で `artifact` は受け取らない
- `ARTIFACT_S3_BUCKET` / `ARTIFACT_S3_KEY` を Lambda 環境変数で固定する

## D. Lambda 実行フロー（確定）

1. 環境変数 `ARTIFACT_S3_BUCKET` / `ARTIFACT_S3_KEY` を取得
2. S3 object を `/tmp/migration-artifact.zip` として取得
3. `/tmp/migration-artifact/` に展開
...
- artifact 取得/展開失敗:
  - `ok=false` で返却
  - `failedStep` を `artifact_download` または `artifact_extract` で返す
- migration/seed 失敗:
...
2. `@pf/functions`
   - migration handler から payload `artifact` 入力を削除
   - S3 取得と zip 展開処理を追加
3. `scripts/migrate.ts`
   - zip 作成と upload 処理を追加
   - invoke payload から `artifact` を除外
4. docs
...
- 2026-04-26: `cdk deploy` 時の baseline artifact upload は採用しない（artifact は `migrate` 実行時に upload）
- 2026-04-26: 任意実行防止のため、artifact の bucket/key は payload 指定ではなく環境変数固定を採用
