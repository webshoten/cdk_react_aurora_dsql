...
- 個別ページは `pages/`、API 呼び出しは `hooks/` に分割する方針

## ステータス（2026-04-28）

- 実装済み（初期）: `11-1.data-01`, `11-2.data-02`
- 設計確定・次実装: `11-3.auth-01`
- 章定義のみ（未着手）: `11-4` 〜 `11-12`
- 仕様メモあり（未実装）: `11-13.maintenance-01`
...
- 2026-04-28: 各ユースケース章を同一テンプレート（対象/目的/初期スコープ/スコープ外/着手条件）で管理する方針を採用
