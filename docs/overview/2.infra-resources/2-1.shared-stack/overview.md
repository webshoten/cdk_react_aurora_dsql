...
- アカウント+リージョン+endpointType（`iot:Data-ATS`）単位で固定される AWS IoT Data endpoint を取得し、SSM Parameter で公開する
...
- `AWS::SSM::Parameter`（`/pf/shared/<sharedEnv>/iot/data-endpoint`）
- `AWS::CloudFormation::CustomResource`（`iot:DescribeEndpoint` 取得用、`endpointType=iot:Data-ATS`）
...
- IoT Data endpoint はアカウント+リージョン+endpointType（`iot:Data-ATS`）単位で固定されるため、shared 層で 1 回だけ `iot:DescribeEndpoint` を呼び、`/pf/shared/<sharedEnv>/iot/data-endpoint` に格納して app stack 群へ SSM 経由で公開する
...
- 2026-05-06: IoT Data endpoint はアカウント+リージョン+endpointType（`iot:Data-ATS`）単位で固定されるため、`SharedIotEndpointConstruct` で `iot:DescribeEndpoint`（`iot:Data-ATS`）を 1 回取得し、`/pf/shared/<sharedEnv>/iot/data-endpoint` に格納して app stack 群へ SSM 経由で公開する方針を追加
