...
- `pnpm qa` の判定は機械実行結果（lint/typecheck/build/scan）を正とし、推測で成功扱いにしない
- `pnpm qa` が失敗した場合は、原因と失敗コマンドを明示し、成功扱いで完了報告しない
- `pnpm qa` は変更対象に応じて観点別 `qa:*` を実行する
  - 常時必須: `qa:static` と `qa:base`
  - `packages/web` 変更時: `qa:frontend`
  - `packages/functions` または `packages/core` 変更時: `qa:backend`
  - `packages/infra` 変更時: `qa:infra`（`CDK_SHARED_ENV` 必須、`CDK_NAG=1` で `cdk-nag` を含む）
  - 認証/認可/秘密情報/IAM/ネットワーク変更時: `qa:security`
- QA運用は「新規指摘を実装時に解消する」を原則とする
  ...
  - 抑制（suppress/ignore/skip）を追加・変更するコミットでは、同一タスクで台帳更新を必須とする
...
- レビュー開始前に、対象コードに対応する `docs/overview`（feature/backend/frontend/infra）を先に確認する
- レビュー着手時に、今回のレビュー観点（3〜6項目）を先に宣言する
- レビューは「設計一致チェック」を先に行い、不一致があれば findings の最優先で報告する
- 認証/認可を含む変更は、token 種別（`idToken` / `accessToken`）と認可境界（Web/API/IoT）を固定観点として毎回確認する
- 失敗時の可観測性（原因追跡ログの有無）と機密情報非出力（token/password/PII）を毎回確認する
- CDK/IaC 変更のレビューでは、手動 CLI 前提になっていないか、再 deploy で収束する冪等性があるかを確認する
- ローカル実行性（開発容易性）を確認する
- ローカル実行性レビューでは、`pnpm dev:*` / `build` / 必須設定（`config.js` / env / SSM）の再現手順が明記されているかを確認する
- 新規依存や起動手順変更がある場合は、`docs/overview/10.local-dev` への反映有無を確認する
- レビュー報告の順序は `Findings（重大度順） -> 根拠ファイル -> 影響 -> 修正案` を基本とする
...
- 2026-05-06: CDK は手動 CLI 前提で補わず、deploy 再実行で収束する冪等設計を必須とする方針を追加
- 2026-05-10: レビュー開始時の設計確認・観点宣言・設計一致チェック・開発容易性（ローカル実行性）確認をレビュールールとして追加
